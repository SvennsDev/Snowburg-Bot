	...
	async execute(interaction) {
        ...
        if (interaction.customId === `artistmodal`) {
            ...
            const ticketcreated = new EmbedBuilder()
                .setTitle(`🖌 | Artist Sollicitatie`)
                .addFields(
                    { name: 'Gemaakt:', value: `${c}`, inline: true },
                    { name: 'Leeftijd:', value: `${age}`, inline: true },
                    { name: 'Waarom wil je een artist worden?', value: `${reason}`, inline: false },
                    { name: 'Wat zijn je sterke/zwakke punten?', value: `${sterk}`, inline: false },
                    { name: 'Wat wil je bereiken als artist?', value: `${bereiken}`, inline: false },
